...
// Элемент принимает объект с элементами вида: {id: 1, authorName: 'Кот Василий', authorPosition: 'менеджер по ловле мышей', authorImg: {alt: 'подпись к изображению', src: '<ccылка на изображение>'} reviewText: 'Это лучшая школа в городе!'} из свойства props.reviews
...
export default function Reviews(props) {
    ...
    let reviewsArrow = props.reviews.map(e => {
        return (<li className='col_box_reviews_list_item' key={e.id}>
    ...
    let scrollReviews = (event) => {
        let target = event.target.closest(".arrow");
        let reviewsList = target.closest('.col_box_reviews').querySelector('.col_box_reviews_list');
