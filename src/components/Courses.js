...
export default class Courses extends Component {
  constructor(props) {
    ...
    this.buttonsList = props.courses.map((e) => <li className="col_box_guitar_button_item" key={e.id}><button className="col_box_guitar_button" onClick={this.changeCourse} data-ident={e.id}>{e.nameButton}</button></li>);
  }

  changeCourse(event) {
    const { target } = event;
    console.log(target.dataset.ident);
    const course = this.props.courses.find((e) => +e.id === +target.dataset.ident);
    this.setState({
      id: course.id,
